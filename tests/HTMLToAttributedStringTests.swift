...
class HTMLToAttributedStringTests: XCTestCase {
    ...
    func testSuperscriptNumber() {
        ...
        guard let baselineOffset = result.attribute(NSAttributedString.Key.baselineOffset, at: 0, effectiveRange: nil) as? CGFloat else {
    ...
    func testStrikethroughWord() {
        ...
        let result = shouldMakeAttributedString(fromHTML: html)
        XCTAssertNotNil(
            result.attribute(NSAttributedString.Key.strikethroughStyle, at: 0, effectiveRange: nil),
...
//        assertSymbolicTraits(.traitBold, foundInFontDescriptorAtIndex: 0, of: result)
    ...
    func testListItemWithFootnoteClassSuperscriptsIndexAndOmitsDot() {
        ...
        result.enumerateAttribute(NSAttributedString.Key(rawValue: superscriptAttributeName), in: NSRange(location: 0, length: result.length), options: []) { (value, range, done) in
    ...
    func testBlockquote() {
        ...
        guard let style = rendered.attribute(NSAttributedString.Key.paragraphStyle, at: 0, effectiveRange: nil) as? NSParagraphStyle else {
    ...
    func testParagraphInBlockquoteIsStillIndented() {
        ...
        guard let style = rendered.attribute(NSAttributedString.Key.paragraphStyle, at: range.location, effectiveRange: nil) as? NSParagraphStyle else {
    ...
    func assertSymbolicTraits(_ trait: UIFontDescriptor.SymbolicTraits, foundInFontDescriptorAtIndex index: Int, of string: NSAttributedString, file: StaticString = #file, line: UInt = #line) {
        guard let font = string.attribute(NSAttributedString.Key.font, at: index, effectiveRange: nil) as? UIFont else {
